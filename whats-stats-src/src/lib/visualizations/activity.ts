...
export function renderActivityOverTime(messages: ChatMessage[], container: HTMLElement) {
  container.innerHTML = '';
...
export function renderActivityOverTimeGrouped(messages: ChatMessage[], container: HTMLElement) {
  container.innerHTML = '';
  const { minDate, maxDate } = getTimeRangeFromMessages(messages);
  if (!minDate || !maxDate) {
    container.textContent = 'No data to display.';
  ...
  if (traces.every((t) => t.y.every((y: number) => y === 0))) {
    container.textContent = 'No data to display.';
    return;
  }
  const L = getPlotlyLayoutSizes();
...
export function renderActivityOverTimeStackedPercent(messages: ChatMessage[], container: HTMLElement) {
  container.innerHTML = '';
  const { minDate, maxDate } = getTimeRangeFromMessages(messages);
  if (!minDate || !maxDate) {
    container.textContent = 'No data to display.';
