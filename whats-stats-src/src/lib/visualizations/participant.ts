...
import { PARTICIPANT_COLORS, getPlotlyLayoutSizes, getChartHeightPx, getPlotlyConfig } from '../utils';
import { getParticipantsSortedByMessageCount } from '../analysis';
import type { ChatMessage } from '../types';

export function renderHourlyActivity(messages: ChatMessage[], container: HTMLElement) {
  container.innerHTML = '';
  const { participants, totalMessagesByUser } = getParticipantsSortedByMessageCount(messages);
  const hourCountsByUser: Record<string, number[]> = {};
  ...
  const hourFractionsByUser: Record<string, number[]> = {};
  participants.forEach((user) => {
...
export function renderLongestStreaks(messages: ChatMessage[], container: HTMLElement) {
  container.innerHTML = '';
  ...
  const layout = {
    ...
    width: container.offsetWidth,
    height: Math.max(
      getChartHeightPx('medium'),
      Math.min(dynamicH, getChartHeightPx('large') * 1.5)
    ),
  };
